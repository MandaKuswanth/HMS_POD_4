...
exports.createAppointment = async (req, res) => {
  try {
    ...
    if (!patient) {
      return res
        .status(404)
        .json(new ApiError(404, "Patient not found with provided UHID"));
    }
    ...
    if (!doctorUser) {
      return res
        .status(404)
        .json(new ApiError(404, "Doctor user account not found"));
    }
    ...
    if (!doctorRole) {
      return res
        .status(500)
        .json(new ApiError(500, "Doctor role not configured"));
    }
    ...
    if (!doctorUser.status || !doctor.status) {
      return res
        .status(400)
        .json(new ApiError(400, "Doctor account is inactive"));
    }
  ...
  try {
    ...
    if (
      ...
      if (!doctor) {
        return res
          .status(404)
          .json(new ApiError(404, "Doctor profile not found"));
      }
      ...
      return res
        .status(403)
        .json(new ApiError(403, "You are not allowed to view appointments"));
    }
    ...
    const doctorIds = appointments.map(
      (appointment) => appointment.doctorEmployeeId,
    );
    ...
    const patientMap = new Map(
      patients.map((patient) => [patient.UHID, patient]),
    );

    const doctorMap = new Map(
      doctors.map((doctor) => [doctor.employeeCode, doctor]),
    );
...
exports.updateAppointment = async (req, res) => {
  try {
    ...
    if (!doctor) {
      return res
        .status(404)
        .json(new ApiError(404, "Assigned doctor not found"));
    }
    ...
    if (date || timeSlot || status) {
      ...
      if (
        existing &&
        ["PENDING", "BOOKED", "IN-PROCESS"].includes(appointment.status)
      ) {
        return res.status(409).json(new ApiError(409, "Slot already booked"));
      }
    }
...
exports.approveAppointment = async (req, res) => {
  try {
    ...
    if (appointment.status !== "PENDING") {
      return res
        .status(400)
        .json(new ApiError(400, "Only pending appointments can be approved"));
    }
...
exports.rejectAppointment = async (req, res) => {
  try {
    ...
    if (appointment.status !== "PENDING") {
      return res
        .status(400)
        .json(new ApiError(400, "Only pending appointments can be rejected"));
    }
