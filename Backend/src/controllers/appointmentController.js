...
exports.getAppointments = async (req, res) => {
    try {
        ...
        if (userPermissions.has("APPOINTMENT_APPROVE")) {
            // Admin / Receptionist — can view all records
        ...
        // Batch-load relational data to prevent N+1 overhead query loops
        const patientIds = [...new Set(appointments.map((a) => a.patientId))]; 
        const doctorIds = [...new Set(appointments.map((a) => a.doctorEmployeeId))]; 

        const [patients, doctors] = await Promise.all([
            Patient.find({ UHID: { $in: patientIds }, isDeleted: false }, { UHID: 1, name: 1, phone: 1, email: 1 }).lean(),
            Employee.find({ employeeCode: { $in: doctorIds }, isDeleted: false }, { employeeCode: 1, name: 1, department: 1, designation: 1 }).lean() 
        ]); 
